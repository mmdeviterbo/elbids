        ...
        <MenuItem 
          ...
          <ListItem alignItems="center" style={{padding:2, gap:"5px"}}>
            ...
            {notif?.type === NOTIFICATION_TYPE.FOLLOWING &&
              <ListItemText
                primary={<Typography variant="subtitle2">{`${titleCase(notif?.post?.item?.buyer?.first_name?.toLowerCase() || "")} placed a bid for ₱${new Intl.NumberFormat().format(notif?.current_bid || notif?.post?.item?.current_bid)}`}</Typography>}
                secondary={<Typography variant="subtitle2" color="textSecondary">{`${notif?.post?.item?.title}`}</Typography>}
            />}
          </ListItem>
        </MenuItem>
